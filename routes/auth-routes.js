...
//redirect?code=.... - the code that we get in query will be later exchanged for profile info. Once it will get the info the passport callback function will be fired. When we arrive to redirect then passport sees that we have the code, it understand that the person already has been to the consent screen, so now we can exhange the code for the profile inforrmation. Passport.authenticate fires the passport callback function before it gets to the router
